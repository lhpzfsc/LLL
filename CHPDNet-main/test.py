...
def seg_precision_recall_f1(pred_mask, gt_mask, eps=1e-8, device='cuda'):
    """
    计算二分类语义分割的 Precision / Recall / F1
    自动处理 numpy 输入 & 放到 GPU
    pred_mask, gt_mask: numpy.ndarray 或 torch.Tensor, shape [B,H,W]，值为0/1
    """
    # 转成 torch 张量并放到 GPU
    ...
    tp = (pred_mask & gt_mask).sum(dim=(1,2)).float()
    fp = (pred_mask & ~gt_mask).sum(dim=(1,2)).float()
    fn = (~pred_mask & gt_mask).sum(dim=(1,2)).float()
    tn = (~pred_mask & ~gt_mask).sum(dim=(1, 2)).float()
...
def validate(val_loader, model, val_seed):
    ...
    for e in range(db_epoch):
        for i, (input, input_name, target, target_b, s_input, s_mask, subcls, class_name, ori_label, ori_label_b, img_cv2) in enumerate(val_loader):
            ...
            output, meta_out = model(s_x=s_input, x_cv2=img_cv2, que_name=input_name, s_y=s_mask, x=input, y_m=target, y_b=target_b,
            ...
            tp, fp, fn, tn = seg_precision_recall_f1(output, target)
            tp, fp, fn, tn = tp.cpu().numpy(), fp.cpu().numpy(), fn.cpu().numpy(), tn.cpu().numpy()
            # print( tp, fp, fn ,tn, tp+fp+fn+tn)
            tp_list.append(tp)
            fp_list.append(fp)
            fn_list.append(fn)
            tp_meter.update(tp), fp_meter.update(fp), fn_meter.update(fn)
    ...
    fp_sum=fp_meter.sum
    fn_sum=fn_meter.sum
    print("tp_sum,fp_sum,fn_sum",tp_sum,fp_sum,fn_sum)
    print("tp_list,fp_list,fn_list", sum(tp_list), sum(fp_list), sum(fn_list))
